...
bool wav::validRIFF(void) const {
	if (memcmp(riff.ChunkID, "RIFF", 4) != 0) {
		#ifdef _DEBUGOUTPUT
		fprintf(stderr, "E: Invalid RIFF header: Format != 'RIFF'\n");
...
bool wav::validFMT(void) const {
	if (memcmp(fmt.SubchunkID, "fmt ", 4) != 0) {
		#ifdef _DEBUGOUTPUT
		fprintf(stderr, "E: Invalid FMT header: Format != 'fmt '\n");
		fprintf(stderr, "\tFormat == %s\n", (char *)fmt.SubchunkID);
		#endif
		return false;
	}
...
bool wav::validFACT(void) const {
	...
	if (memcmp(fact->SubchunkID, "fact", 4) != 0) {
		#ifdef _DEBUGOUTPUT
		fprintf(stderr, "E: Invalid FACT header: Format != 'fact'\n");
...
bool wav::validDATA(void) const {
	if (memcmp(data.SubchunkID, "data", 4) != 0) {
		#ifdef _DEBUGOUTPUT
		fprintf(stderr, "E: Invalid DATA header: Format != 'data'\n");
